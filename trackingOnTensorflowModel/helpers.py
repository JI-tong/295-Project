...
def draw_box_label(img, bbox_cv2, num_cars, box_color=(0, 255, 255), show_label=True):
    '''
    ...
    if show_label:
        ...
        cv2.rectangle(img, (left-2, top-35), (right+2, top), box_color, -1, 1)
        
        # Output the labels that show the x and y coordinates of the bounding box center.
        text_x= 'x='+str((left+right)/2)
        cv2.putText(img,text_x,(left,top-15), font, font_size, font_color, 1, cv2.LINE_AA)
        text_y= 'y='+str((top+bottom)/2)
        cv2.putText(img,text_y,(left,top-5), font, font_size, font_color, 1, cv2.LINE_AA)
        text_id = 'car_id='+str(num_cars)
